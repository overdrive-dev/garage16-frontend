...
// Componente para a lista de veículos
function VehicleList({ filtros }) {
  ...
  useEffect(() => {
    const loadVeiculos = async () => {
      ...
      try {
        ...
        const dados = mockAnuncios.publicados.filter(anuncio => {
  ...
  if (veiculos.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">Nenhum veículo encontrado com os filtros selecionados.</p>
      </div>
    );
  }
          ...
          <div className="flex gap-8">
            ...
            <Suspense fallback={<FiltersSkeleton />}>
              <Filters 
                ...
                filtroSelecionadoId={filtroSelecionadoId}
                handleLoadFiltro={handleLoadFiltro}
                handleDeleteClick={handleDeleteClick}
                handlePrecoChange={handlePrecoChange}
                handleAnoChange={handleAnoChange}
                temFiltrosAtivos={temFiltrosAtivos}
                temAlteracoes={temAlteracoes}
                handleUpdateFiltro={handleUpdateFiltro}
